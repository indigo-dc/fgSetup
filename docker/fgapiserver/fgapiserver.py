...
app = Flask(__name__)
app.config.update({
    'SECRET_KEY': 'XXXXXXYYYYYYZZZZZZZ00000001111122222',
    'TESTING': True,
    'DEBUG': True,
...
#    'OIDC_OPENID_REALM': 'fgapiserver@CT',
...
def oidc_info():
    access_token = None
    oidc_info = oidc.user_getinfo(['name', 'email', 'sub'])
    username = oidc_info.get('name')
    email = oidc_info.get('email')
    user_id = oidc_info.get('sub')
    if user_id in oidc.credentials_store:
        try:
            from oauth2client.client import OAuth2Credentials
            access_token = OAuth2Credentials.from_json(oidc.credentials_store[user_id]).access_token
            logger.debug("OIDC access_token='%s'" % access_token)
            #headers = {'Authorization': 'Bearer %s' % (access_token)}
            #greeting = requests.get('http://localhost:3000/api', headers=headers).text
        except:
            print "Could not get access token"


    return ("""OIDC user %s email is %s and your user_id is %s, access token '%s'""" %
            (username, email, user_id, access_token))

@app.route('/oidc', methods=['GET', 'POST'])
@app.route('/%s/oidc' % fgapiver, methods=['GET', 'POST'])
@oidc.require_login
def oidc():
    return oidc_info()

@app.route('/oidc_callback', methods=['GET', 'POST'])
def oidc_callback():
    return oidc_info()
